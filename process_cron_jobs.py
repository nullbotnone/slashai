...
def analyze_job(job):
    ...
    if interval_ms != float('inf') and time_since_last_run_ms > 2 * interval_ms:
        issues.append(f"Not run in over 2x interval ({ms_to_datetime_str(time_since_last_run_ms)} > 2x {format_duration(interval_ms)})")
    ...
    return {
        ...
        'time_since_last_run_str': ms_to_datetime_str(time_since_last_run_ms) if time_since_last_run_ms != float('inf') else "Never",
