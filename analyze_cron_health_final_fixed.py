...
elif any(job['state'].get('consecutiveErrors', 0) > CONSECUTIVE_ERROR_THRESHOLD for job in jobs):
    analysis['overall_health'] = 'poor'
elif any(job['state'].get('lastDurationMs', 0) > LONG_RUN_TIME_MS for job in jobs):
    analysis['overall_health'] = 'poor'
...
for job in analysis['job_details']:
    last_run_str = datetime.datetime.fromtimestamp(job['last_run_at_ms']/1000, datetime.timezone.utc).strftime('%Y-%m-%d %H:%M:%S %Z') if job['last_run_at_ms'] > 0 else 'Never'
    time_since_str = f"{job['time_since_last_run_hours']:.2f} hours ({job['time_since_last_run_hours']*60:.0f} minutes)" if job['time_since_last_run_hours'] is not None else 'N/A'
    
    report += f"""### {job['name']} (ID: {job['id']})
- **Status:** {'ENABLED' if job['enabled'] else 'DISABLED'}
- **Last Run:** {last_run_str} ({job['last_run_status']})
- **Consecutive Errors:** {job['consecutive_errors']}
- **Last Run Duration:** {job['last_duration_hours']:.2f} hours ({job['last_run_duration_ms']} ms)
- **Time Since Last Run:** {time_since_str}
