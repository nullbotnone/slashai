...
def main():
    ...
    # Check if the SlashAI Daily Tool Check job has consecutive errors and trigger if needed
    ...
    if daily_tool_job:
        ...
        if consecutive_errors > 0:
            print(f"SlashAI Daily Tool Check job has {consecutive_errors} consecutive errors. Manually triggering...")
            try:
                subprocess.run(['openclaw', 'cron', 'run', daily_tool_job_id], check=True)
                print("Successfully triggered the SlashAI Daily Tool Check job.")
            except subprocess.CalledProcessError as e:
                print(f"Failed to trigger the job: {e}")
        else:
            print("SlashAI Daily Tool Check job has no consecutive errors. No need to trigger.")
    else:
