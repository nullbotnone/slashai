...
# Load the live cron jobs data
with open('/home/rpi/.openclaw/workspace/projects/slashai/cron_jobs_live.json', 'r') as f:
    data = json.load(f)

jobs = data['jobs']

# Use current time from the health monitor job's runningAtMs (which is very close to now)
# Or we can use system time. Let's use system time for consistency.
dt_utc = datetime.now(timezone.utc)
current_time_ms = int(dt_utc.timestamp() * 1000)

# Alternatively, we could extract the current time from the running health monitor job
# But using system time is fine for this purpose.
...
if daily_tool_check_job:
    ...
    if consecutive_errors > 0:  # If there are any consecutive errors
        report_lines.append("")
        ...
        report_lines.append("")
        report_lines.append("### Manual Trigger Execution:")
        report_lines.append("Since we cannot directly trigger cron jobs via API in this context,")
        report_lines.append("we would normally use the sessions_spawn tool to create an agent turn")
        report_lines.append("with the same payload as the job.")
        report_lines.append("")
        report_lines.append("**Job Payload:**")
        report_lines.append(daily_tool_check_job['payload']['message'])
        report_lines.append("")
        report_lines.append("**Note:** In a real implementation, we would execute:")
        report_lines.append("```")
        report_lines.append("sessions_spawn({")
        report_lines.append('  task: "<payload message>",')
        report_lines.append('  label: "Manual trigger of SlashAI Daily Tool Check",')
        report_lines.append('  runtime: "subagent"')
        report_lines.append("})")
        report_lines.append("```")
    else:
        report_lines.append("")
        report_lines.append(f"The SlashAI Daily Tool Check job (ID: {daily_tool_check_id}) has no consecutive errors (consecutive errors: {consecutive_errors}).")
...
with open(log_path, 'a') as f:
    f.write(f"[{datetime.now().isoformat()}] Cron health check completed. Report saved to {report_path}. Issues found: {len(issues)}. Daily tool check consecutive errors: {daily_tool_check_job['state'].get('consecutiveErrors', 0) if daily_tool_check_job else 'unknown'}\\n")
