...
if any(job['state'].get('consecutiveErrors', 0) > 2 for job in jobs):
    print("2. Investigate jobs with consecutive errors > 2.")
if any(job['state'].get('lastRunStatus') == 'error' for job in jobs):
...
# Generate markdown report as requested
markdown_report = f"""# SlashAI Cron Health Check Report

**Generated at:** {datetime.datetime.fromtimestamp(current_time_ms/1000, tz=datetime.timezone.utc).strftime('%Y-%m-%d %H:%M:%S %Z')}
**Current time (provided):** Wednesday, June 10th, 2026 - 18:02 (America/Chicago) / 2026-06-10 23:02 UTC
...
- **Last run:** {datetime.datetime.fromtimestamp(issue['last_run_at_ms']/1000, tz=datetime.timezone.utc).strftime('%Y-%m-%d %H:%M:%S %Z') if issue['last_run_at_ms'] > 0 else 'Never'}
- **Last run status:** {issue['last_run_status']}
- **Consecutive errors:** {issue['consecutive_errors']}
- **Last run duration:** {issue['last_duration_ms']/(1000*60):.1f} minutes
- **Time since last run:** {issue['time_since_last_run_hours']:.1f} hours (if applicable)
...
"""
    ...
    markdown_report += "No issues found - all jobs are healthy.\n"

markdown_report += """
## Recommended Actions
"""
if any(not job['enabled'] for job in [j for j in jobs if j['id'] in [i['id'] for i in issues]]):
    markdown_report += "1. Enable any disabled jobs that should be running.\n"
if any(job['state'].get('consecutiveErrors', 0) > 2 for job in jobs):
    markdown_report += "2. Investigate jobs with consecutive errors > 2.\n"
if any(job['state'].get('lastRunStatus') == 'error' for job in jobs):
    markdown_report += "3. Check logs for jobs that last ran with error status.\n"
if any((current_time_ms - job['state'].get('lastRunAtMs', 0)) > 2 * get_interval_ms(job['schedule']['expr']) for job in jobs if job['state'].get('lastRunAtMs', 0) > 0):
    markdown_report += "4. Check why some jobs haven't run in over 2x their scheduled interval.\n"
if any(job['state'].get('lastDurationMs', 0) > 3600000 for job in jobs):
    markdown_report += "5. Investigate jobs with excessively long run times (>1 hour).\n"
...
if daily_tool_job:
    ...
    if consecutive_errors > 0:
        markdown_report += f"\n6. SlashAI Daily Tool Check job has {consecutive_errors} consecutive errors.\n"
        markdown_report += "   As per instructions, manually triggering it to test if underlying issue is resolved...\n"
        markdown_report += "   [Manual trigger would be executed here]\n"
    else:
        markdown_report += f"\n6. SlashAI Daily Tool Check job has {consecutive_errors} consecutive errors - no action needed.\n"

markdown_report += f"""
---
*Report generated by SlashAI Cron Health Monitor*
"""
...
print(f"Markdown report saved to: /home/rpi/.openclaw/workspace/projects/slashai/cron-health-report.md")